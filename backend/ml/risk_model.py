...
def predict_risk(profile: dict) -> dict:
    ...
    if fallback_fields:
        data_source_warning = (
            f"This model was trained on UK Open University data (OULAD, 2013-14 cohorts). "
            f"The following fields didn't match known categories and used a default: "
            f"{', '.join(fallback_fields)}. Treat this prediction as indicative, not precise."
        )
...
def explain_prediction(profile: dict) -> dict:
    ...
    # Base value (model bias) — in log-odds space (margin), not probability.
    # SHAP values are in the same log-odds space, so base + sum(contribs)
    # equals the log-odds prediction, NOT the probability shown to the user.
    ...
    return {
        ...
        "summary": (
            ...
            f"Note: SHAP values are in log-odds space and show relative feature importance, "
            f"not absolute probability contributions."
        ),
    }
