...
def _load():
    ...
    logger.info(
        "Course embeddings loaded: %d courses, %d dims",
...
def recommend(goal_text: str, top_k: int = 5) -> list:
    """
    Recommend courses based on cosine similarity between learner goal
    ...
    similarities = np.dot(_embeddings, goal_embedding.T).squeeze()
