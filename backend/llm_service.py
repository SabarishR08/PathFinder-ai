...
def _detect_domain(message: str, domain_options: list[str]) -> Optional[str]:
    ...
    # Scan ALL buckets to count keyword hits per domain, rather than
    # returning on the first hit. This prevents a strong keyword in one
    # domain from locking out a better match in another when the message
    # contains signals for multiple domains.
    ...
    if len(domain_hits) == 1:
        # Single domain matched — return immediately (fast path)
        matched_domain = next(iter(domain_hits))
        logger.info("Pass 1 keyword match: %s → '%s'", domain_hits, matched_domain)
        return matched_domain
    elif len(domain_hits) > 1:
        # Multiple domains tied — let the LLM break the tie
        tied_domains = list(domain_hits.keys())
        logger.info("Pass 1 multi-domain tie: %s — asking LLM to disambiguate", domain_hits)
        # Reorder tied domains: AI-specific first, then others
        priority = ["AI Engineering", "Prompt Engineering & LLMOps", "Generative AI & RAG Systems"]
        reordered = [d for d in priority if d in tied_domains]
        reordered += [d for d in tied_domains if d not in reordered]
        ...
        if not raw.startswith("LLM_ERROR"):
            raw = raw.strip().strip('"').strip("'")
            for d in reordered:
                if d.lower() == raw.lower():
                    logger.info("LLM disambiguation: '%s' → '%s' (from tie %s)", raw, d, list(domain_hits.keys()))
                    return d
            for d in reordered:
                if d.lower() in raw.lower() or raw.lower() in d.lower():
                    logger.info("LLM disambiguation (substring): '%s' → '%s' (from tie %s)", raw, d, list(domain_hits.keys()))
                    return d
        # LLM failed to disambiguate — return the domain with most hits
        best = max(domain_hits, key=domain_hits.get)
        logger.info("LLM disambiguation failed, falling back to highest-hit domain: '%s'", best)
        return best

    # ── LLM fallback for ambiguous cases (no keyword matches at all) ─────────
