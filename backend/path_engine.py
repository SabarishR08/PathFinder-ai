...
Two path-generation algorithms are provided:
  ...
  - generate_optimal_path(): Kahn's algorithm with min-heap (SPT scheduling)
    over the same DAG. Each skill's edge weight is its real average course
    duration (in months, parsed from actual course Duration fields). When
    multiple independent skills become available, the cheaper one is scheduled
    first — a provably-optimal heuristic for minimizing average completion
    time (Smith's rule / Critical Path Method). NOT Dijkstra: the graph has
    AND-semantics (all prerequisites mandatory), so there is no shorter
    alternative path to discover.
...
class PathEngine:
    ...
    def find_skill_id_by_name(self, domain: str, name_fragment: str):
        """Best-effort fuzzy lookup: exact match first, then longest substring match."""
        if domain not in self.graph:
            return None
        ...
        # Collect ALL substring matches (both directions)
        matches = []
        for s in skills:
            skill_lower = s["name"].lower()
            if name_fragment_l in skill_lower or skill_lower in name_fragment_l:
                # Prefer prefix matches over mid-string matches
                is_prefix = skill_lower.startswith(name_fragment_l) or name_fragment_l.startswith(skill_lower)
                matches.append((s["id"], s["name"], len(s["name"]), is_prefix))

        if not matches:
            return None

        # Sort: longest name first, prefix matches before mid-string matches
    ...
    # Keywords that indicate an ML-related goal (moved from Dashboard.jsx
    # so domain logic lives in the backend, not the UI).
    ML_GOAL_KEYWORDS = [
        "machine learning", "deep learning", "tensorflow", "neural",
        "ai ", "artificial intelligence",
    ]

    def is_ml_goal(self, goal_name: str) -> bool:
        """Return True if the goal name suggests a Machine Learning goal."""
        if not goal_name:
            return False
        lower = goal_name.lower()
        return any(kw in lower for kw in self.ML_GOAL_KEYWORDS)
