...
const correctLoop = `      for await (const part of result.fullStream) {
...
code = code.replace(/      for await \(const part of result\.fullStream\) \{[\s\S]*?      \}\n\n      if \(!replyBuffer\.trim\(\)\) \{/, correctLoop + '\n\n      if (!replyBuffer.trim()) {');
